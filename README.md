...
_ds_ plugin which consumes a `prediction-default.json` from an upstream COVIDnet prediction file and depending on content, might throw an execption.


## Abstract

This _meta_ plugin is part of a COVIDnet analysis compute graph. If an upstream processing has inferred COVID infection, this node will throw an exception and terminate. Within the context of the ChRIS UI, this has the effect of coloring the Feed Graph red in the visual representation - in effect providing a simple visual tag on the final result of the compute graph.
